...
class TopGbHdmi(gowinDviTx: Boolean = true) extends RawModule {
    ...
    /* gb pad output */
    val P10= IO(Output(Bool())) //gbpad_a and right
    val P11= IO(Output(Bool())) //gbpad_b and left
    val P12= IO(Output(Bool())) //gbpad_select and up
    val P13= IO(Output(Bool())) //gbpad_start and down
    val gbpad_right  = IO(Output(Bool()))
    val gbpad_left   = IO(Output(Bool()))
    val P14 = IO(Input(Bool())) // direction pad
    val P15 = IO(Input(Bool())) // buttons and config
    /********************************************/
